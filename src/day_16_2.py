...
def walk_all_paths(graph):
    ...
    def traverse_graph(graph, current_node, minutes_left, path, total_value):
        ...
        if minutes_left <= 0 or len(v_left) == 0:
            ...
            return
        potential_values = determine_values(graph, current_node, minutes_left, v_left)
...
def find_paths(graph):
    ...
    while minutes_left > 0 or elephant_minutes_left > 0:
        v_left = valves_left(graph)
        potential_values = determine_values(graph, elephant_current_node, elephant_minutes_left, v_left)
        if potential_values:
            highest_value_node = max(potential_values, key=potential_values.get)
            shortest_path = TRAVEL_PATHS[(elephant_current_node, highest_value_node)]
            elephant_minutes_left -= len(shortest_path)
            total_score += potential_values[highest_value_node]
            graph.nodes[highest_value_node]['state'] = 'open'
            elephant_current_node = highest_value_node
