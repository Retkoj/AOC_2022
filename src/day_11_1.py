...
class MonkeyGame:
    ...
    def play_round(self):
        for n in range(0, self.n_monkeys):
            ...
            updated_items = [math.floor(monkey.do_operation(item) / 3) for item in monkey.get_all_items()]
            self.monkeys[monkey.test['true']].items += [item for item in updated_items if
                                                        item % monkey.test['divisible_by'] == 0]
            self.monkeys[monkey.test['false']].items += [item for item in updated_items if
                                                         item % monkey.test['divisible_by'] != 0]
