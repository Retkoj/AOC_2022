...
class CrateStacks:
    ...
    def move_crate(self, move_from, move_to):
        crate = self.get_crate_from_stack(move_from)
        if move_to in self.stacks.keys():
            # when in play, add to end
            self.stacks[move_to].append(crate)
        else:
            self.stacks[move_to] = [crate]
...
def make_move(move_line, crate_stacks: CrateStacks):
    ...
    for _ in range(int(move[0])):
        crate_stacks.move_crate(int(move[1]), int(move[2]))
